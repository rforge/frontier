...
all.equal( sbb5ecf[-39], bb5ecf[-39], tol = 1e-4 )
...
bb5eef <- frontier( data = as.data.frame( riceProdPhil ),
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zNames = NA )
all.equal( sbb5eef[-39], bb5eef[-39], tol = 1e-4 )
tmp <- efficiencies( sbb5eef, margEff = TRUE )

# Comparisons
round( rbind( coef( bb5ecf ), coef( bb5eef ) ), 3 )
all.equal( coef( bb5ecf ), coef( bb5eef ), tol = 1e-4 )
all.equal( vcov( bb5ecf ), vcov( bb5eef ), tol = 1e-4 )
all.equal( efficiencies( bb5ecf ), efficiencies( bb5eef ), tol = 1e-4 )
...
all.equal( sbb6ecf[-39], bb6ecf[-39], tol = 1e-4 )
...
all.equal( sbb6eef[-39], bb6eef[-39], tol = 1e-4 )
tmp <- efficiencies( sbb6eef, margEff = TRUE )

# Comparisons
round( rbind( coef( bb6ecf ), coef( bb6eef )[ c( 1:4, 6:7, 5 ) ] ), 3 )
all.equal( efficiencies( bb6ecf ), efficiencies( bb6eef ), tol = 1e-4 )
...
b5eef <- frontier( data = riceProdPhil,
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zNames = NA )
all.equal( sb5eef[-39], b5eef[-39], tol = 1e-4 )
all.equal( b5eef[ -c( 4, 5, 20, 31, 39 ) ], bb5eef[ -c( 4, 5, 20, 31, 39 ) ], 
   tol = 1e-4 )
all.equal( c( t( residuals( b5eef ) ) ), c( residuals( bb5eef ) ), tol = 1e-4 )

# Comparisons
round( rbind( coef( b5ecf ), coef( b5eef ) ), 3 )
all.equal( coef( b5ecf ), coef( b5eef ), tol = 1e-4 )
all.equal( vcov( b5ecf ), vcov( b5eef ), tol = 1e-4 )
...
all.equal( sb6ecf[-39], b6ecf[-39], tol = 1e-4 )
all.equal( b6ecf[-39], bb6ecf[-39], tol = 1e-4 )
...
b6eef <- frontier( data = riceProdPhil,
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zIntercept = TRUE, zNames = NA )
all.equal( sb6eef[-39], b6eef[-39], tol = 1e-4 )
all.equal( b6eef[ -c( 4, 5, 20, 31, 39 ) ], bb6eef[ -c( 4, 5, 20, 31, 39 ) ], 
   tol = 1e-4 )
all.equal( c( efficiencies( b6ecf ) ), c( efficiencies( bb6eef ) ), tol = 1e-4 )
all.equal( c( residuals( b6ecf ) ), c( residuals( bb6eef ) ), tol = 1e-4 )

# Comparisons
round( rbind( coef( b6ecf ), coef( b6eef )[ c( 1:4, 6:7, 5 ) ] ), 3 )
all.equal( c( efficiencies( b6ecf ) ), c( t( efficiencies( b6eef ) ) ), 
   tol = 1e-4 )
all.equal( c( residuals( b6ecf ) ), c( t( residuals( b6eef ) ) ), tol = 1e-4 )

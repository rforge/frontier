...
all.equal( sbb5ecf[-36], bb5ecf[-36] )
...
bb5eef <- frontier( data = as.data.frame( riceProdPhil ),
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zNames = NA )
all.equal( sbb5eef[-36], bb5eef[-36] )
...
all.equal( sbb6ecf[-36], bb6ecf[-36] )
...
all.equal( sbb6eef[-36], bb6eef[-36] )
...
b5eef <- frontier( data = riceProdPhil,
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zNames = NA )
all.equal( sb5eef[-36], b5eef[-36] )
all.equal( b5eef[ -c( 3, 4, 16, 26, 36 ) ], bb5eef[ -c( 3, 4, 16, 26, 36 ) ] )
...
all.equal( sb6ecf[-36], b6ecf[-36] )
all.equal( b6ecf[-36], bb6ecf[-36] )
...
b6eef <- frontier( data = riceProdPhil,
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zIntercept = TRUE, zNames = NA )
all.equal( sb6eef[-36], b6eef[-36] )
all.equal( b6eef[ -c( 3, 4, 16, 26, 36 ) ], bb6eef[ -c( 3, 4, 16, 26, 36 ) ] )

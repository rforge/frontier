...
all.equal( sbb5ecf[-33], bb5ecf[-c(31,34)] )
...
bb5eef <- frontier( data = as.data.frame( riceProdPhil ),
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zNames = NA )
all.equal( sbb5eef[-33], bb5eef[-c(31,34)] )
...
all.equal( sbb6ecf[-33], bb6ecf[-c(31,34)] )
...
all.equal( sbb6eef[-33], bb6eef[-c(31,34)] )
...
b5eef <- frontier( data = riceProdPhil,
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zNames = NA )
all.equal( sb5eef[-33], b5eef[-c(31,34)] )
...
all.equal( sb6ecf[-33], b6ecf[-c(31,34)] )
...
b6eef <- frontier( data = riceProdPhil,
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zIntercept = TRUE, zNames = NA )
all.equal( sb6eef[-33], b6eef[-c(31,34)] )

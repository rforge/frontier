...
all.equal( sbb5ecf[-40], bb5ecf[-40], tol = 1e-3 )
...
bb5eef <- frontier( data = as.data.frame( riceProdPhil ),
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zNames = NA )
all.equal( sbb5eef[-40], bb5eef[-40], tol = 1e-3 )
...
all.equal( sbb6ecf[-40], bb6ecf[-40], tol = 1e-3 )
...
all.equal( sbb6eef[-40], bb6eef[-40], tol = 1e-3 )
...
b5eef <- frontier( data = riceProdPhil,
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zNames = NA )
all.equal( sb5eef[-40], b5eef[-40], tol = 1e-3 )
all.equal( b5eef[ -c( 4, 5, 20, 31, 32, 40 ) ], bb5eef[ -c( 4, 5, 20, 31, 32, 40 ) ], 
   tol = 1e-3 )
...
all.equal( sb6ecf[-40], b6ecf[-40], tol = 1e-3 )
all.equal( b6ecf[-40], bb6ecf[-40], tol = 1e-3 )
...
b6eef <- frontier( data = riceProdPhil,
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zIntercept = TRUE, zNames = NA )
all.equal( sb6eef[-40], b6eef[-40], tol = 1e-3 )
all.equal( b6eef[ -c( 4, 5, 20, 31, 32, 40 ) ], bb6eef[ -c( 4, 5, 20, 31, 32, 40 ) ], 
   tol = 1e-3 )

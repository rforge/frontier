...
all.equal( sbb5ecf[-37], bb5ecf[-37] )
...
bb5eef <- frontier( data = as.data.frame( riceProdPhil ),
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zNames = NA )
all.equal( sbb5eef[-37], bb5eef[-37] )
...
all.equal( sbb6ecf[-37], bb6ecf[-37] )
...
all.equal( sbb6eef[-37], bb6eef[-37] )
...
b5eef <- frontier( data = riceProdPhil,
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zNames = NA )
all.equal( sb5eef[-37], b5eef[-37] )
all.equal( b5eef[ -c( 3, 4, 16, 26, 37 ) ], bb5eef[ -c( 3, 4, 16, 26, 37 ) ] )
...
all.equal( sb6ecf[-37], b6ecf[-37] )
all.equal( b6ecf[-37], bb6ecf[-37] )
...
b6eef <- frontier( data = riceProdPhil,
   yName = "lPROD", xNames = c( "lAREA", "lLABOR", "lNPK" ),
   zIntercept = TRUE, zNames = NA )
all.equal( sb6eef[-37], b6eef[-37] )
all.equal( b6eef[ -c( 3, 4, 16, 26, 37 ) ], bb6eef[ -c( 3, 4, 16, 26, 37 ) ] )
